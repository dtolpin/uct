...
\frenchspacing
\pdfinfo{
/Title (Rational Monte Carlo Sampling in Trees)
...
it is much better to minimize the {\em simple} regret.  However, the
...
best arm, and, by extension, the best action in Monte Carlo tree
search. Monte Carlo tree search is used to solve Markov Decision
Processes (MDP) approximately. An MDP is defined by the set of states
$S$, the set of actions $A$, the transition table $T(s, a, s')$, the
reward table $R(s, a, s')$, the initial state $s$ and the goal state
$t$: $(S, A, T, R, s, t)$ \cite{Russell.aima}.  Monte Carlo tree
search explores an MDP by \emph{rollouts}---trajectories from the
...
arm with equal probability $\frac {1-\varepsilon} {K-1}$. 
\end{dfn}
...
We suggest an improvement on the UCT algorithm, which combines
different sampling schemes on the first step and during the rest of a rollout:
\begin{dfn}
...
This scheme outperforms the UCT sampling scheme that selects actions based on
UCB only. Additionaly, such two-stage sampling scheme is significantly less
sensitive to the tuning of the exploration factor $\alpha$ of UCT,
since the conflict \cite{Bubeck.pure}
between the need for a larger value of $\alpha$ on the first step (simple
...
steps but a larger value of $\alpha$ for the first step than for the rest
...
\begin{figure}[t]
  \centering
  \includegraphics[scale=0.35,trim=0 0 0 16pt,clip]{flat-trilevel-vct-k=64.pdf}\\
