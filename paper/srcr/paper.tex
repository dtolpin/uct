...
generated 2-level trees, and on the sailing domain, showing where the proposed schemes show improved
...
Much of research literature on Multi-armed bandits centered
on minimizing the cumulative regret. The
UCB scheme (see below) was shown to be near-optimal in minimizing the cumulative regret \cite{Auer.ucb}. 
...
$\beta $ to be better than the current best \cite{Russell.aima}. The ``cost'' of search
...
\begin{itemize}
\item Defining the cost of a sample is not trivial.
...
We will denote realizations of SR+CR as \emph{Alg}+UCT, where
\emph{Alg} is the sampling scheme employed at the first step of a
rollout (e.g. $\frac 1 2$-greedy+UCT). Such two-stage sampling scheme
outperforms UCT, and, additionally, is
