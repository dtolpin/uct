...
which outperforms UCT empirically. Optimizing the sampling process is a
metareasoning problem. However, applying the theory of VOI for search
to MCTS is non-trivial. We nevertheless propose a sampling scheme
that is ``aware'' of VOI, achieving an algorithm that outperforms 
...
collect only the reward of the last pull. Strategies that minimize the
simple regret are called pure exploration
...
principles of {\bf bounded rationality} \cite{Horvitz.reasoningabout} and
{\bf rational metareasoning} \cite{Russell.right}. In search, under myopic
and sub-tree independence assumptions, one maintains a current best
move $\alpha $ at the root, and finds the expected gain from finding
another move $\beta $ to be better than the current best
\cite{Russell.right}. The ``cost'' of search actions can also be
factored in.  Ideally, an ``optimal'' sampling scheme, to be used for
selecting what to sample, both at the root node \cite{HayRussell.MCTS}
and elsewhere, can be developed using metareasoning.  However, this
ultimate goal is extremely difficult to achieve. We introduce in this
paper simple schemes more amenable to analysis, loosely based on the
metareasoning concept of value of information, and compare them to UCB
...
maximizes $b_i$, where:
\begin{equation}
b_i=\overline X_i+\sqrt {{c \sqrt n} / {n_i}}
\end{equation}
...
2\gamma\sum_{i=1}^K\Delta_i\exp\left(-{c\sqrt{n}}/2\right)
\end{equation}
with probability at least $1-\eta$.
\end{thm}
...
mark. Our suggested improvement to UCT thus combines different sampling
schemes on the first step and during the rest of each rollout:
\begin{dfn}
...
rollout (e.g. $\frac 1 2$-greedy+UCT). We expect such two-stage sampling schemes to
